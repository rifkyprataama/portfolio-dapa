...
export function Sidebar() {
  ...
  return (
    // Lebar sidebar dikembalikan ke w-64 agar lebih proporsional
    <aside className="w-full md:w-64 flex flex-col gap-8 shrink-0 transition-colors duration-500 pb-8 md:sticky md:top-10 self-start z-10">
      ...
      {/* Profil: Diperbesar menjadi w-28 h-28 */}
      ...
      <nav className="flex-1 flex flex-col justify-between mt-2">
        {/* Jarak antar menu dilonggarkan menjadi space-y-1.5 */}
        <ul className="space-y-1.5 list-none m-0 p-0">
            ...
            return (
              <li key={link.label}>
                <Link 
                  href={link.href} 
                  className={cn(
                    // Teks dibesarkan ke text-[15px] dan padding px-4 py-2.5
                    "group flex items-center justify-between px-4 py-2.5 rounded-lg text-[15px] font-medium transition-all duration-300 ease-out",
                    isActive 
          ...
          <li className="pt-5 mt-4 border-t border-zinc-200 dark:border-zinc-800/50">
            <button
              ...
              className="w-full group flex items-center justify-between px-4 py-2.5 rounded-lg text-[15px] font-medium transition-all duration-300 ease-out text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800/40 hover:text-zinc-900 dark:hover:text-zinc-200"
            >
              <div className="flex items-center gap-3.5">
              ...
              <div className="w-5 h-5 rounded flex items-center justify-center bg-zinc-200 dark:bg-zinc-800 text-zinc-500 font-mono font-bold text-xs">
                K
              </div>
            </button>
          </li>
        ...
        {/* Teks Copyright sedikit dibesarkan agar mudah dibaca */}

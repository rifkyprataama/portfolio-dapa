...
export function WakatimeStats() {
  ...
  const topLanguages = [
    ...
    { name: "Markdown", percent: 15 },
    { name: "XML", percent: 5 },
    { name: "JSON", percent: 5 },
  ]

  const topEditors = [
    { name: "VS Code", percent: 76 },
    { name: "Unknown Editor", percent: 15 },
    { name: "Android Studio", percent: 9 },
  ]
    ...
    <div className="flex flex-col gap-6 w-full mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[
          ...
          <div key={i} className="flex flex-col justify-center p-5 border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-900/20 rounded-2xl hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
      ...
      {/* Grid Progress Bars (Languages & Editors) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Top Languages */}
          ...
          <div className="flex flex-col gap-4">
            {topLanguages.map((lang, i) => (
              <div key={i} className="flex items-center gap-3 text-sm">
                <span className="font-bold text-zinc-900 dark:text-zinc-100 w-24 shrink-0">{lang.name}</span>
                <div className="flex-1 h-2 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden">
                  <div className="h-full bg-yellow-400 dark:bg-yellow-500 rounded-full" style={{ width: `${lang.percent}%` }}></div>
                </div>
        ...
        <div className="flex flex-col p-5 border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-900/20 rounded-2xl">
          ...
          <div className="flex flex-col gap-4">
            {topEditors.map((editor, i) => (
              <div key={i} className="flex items-center gap-3 text-sm">
                <span className="font-bold text-zinc-900 dark:text-zinc-100 w-24 shrink-0 truncate">{editor.name}</span>
                <div className="flex-1 h-2 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden">
                  <div className="h-full bg-yellow-400 dark:bg-yellow-500 rounded-full" style={{ width: `${editor.percent}%` }}></div>
                </div>

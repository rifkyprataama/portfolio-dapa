...
import { Search, ChevronDown, Plus, Link as LinkIcon, X, ArrowRight } from "lucide-react"
...
export function AchievementsList() {
  ...
  const uniqueTypes = ["All", ...Array.from(new Set(d.achievementsData.map(item => item.type)))]
  const uniqueCategories = ["All", ...Array.from(new Set(d.achievementsData.map(item => item.category)))]

  const filteredData = useMemo(() => {
    return d.achievementsData.filter((item) => {
  ...
  }, [d.achievementsData, searchQuery, typeFilter, categoryFilter])
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-2">
        ...
        <div className="relative w-full md:w-48 shrink-0">
          <select 
            ...
            className="w-full appearance-none pl-4 pr-10 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-900 dark:focus:ring-zinc-100 transition-all cursor-pointer"
          >
            {uniqueTypes.map(type => (
              <option key={type} value={type}>{type === "All" ? d.filterType : type}</option>
            ))}
          </select>
        ...
        <div className="relative w-full md:w-48 shrink-0">
          ...
          >
            {uniqueCategories.map(cat => (
              <option key={cat} value={cat}>{cat === "All" ? d.filterCategory : cat}</option>
            ))}
          </select>
          <ChevronDown className="absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400 pointer-events-none" />
        </div>
        ...
        {filteredData.map((item) => (
          <div key={item.id} className="flex flex-col border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 group">
            ...
            <div 
              ...
              className="aspect-[4/3] w-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden relative border-b border-zinc-200 dark:border-zinc-800 cursor-pointer"
            >
            ...
            <div className="flex flex-col flex-1 p-5">
               <p className="text-[10px] font-mono text-zinc-500 mb-2">{item.credentialId}</p>
               
               <h3 
                 onClick={() => setSelectedItem(item)}
                 className="text-[15px] font-bold text-zinc-900 dark:text-zinc-100 leading-snug line-clamp-2 mb-1 cursor-pointer hover:underline decoration-zinc-300 dark:decoration-zinc-600 underline-offset-4 transition-all"
               >
                 {item.title}
               </h3>
               ...
               <div className="flex flex-wrap gap-2 mt-auto mb-5">
                 <span className="px-2.5 py-1 rounded-full border border-zinc-200 dark:border-zinc-700 text-[11px] font-medium text-zinc-600 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-800/50">
                   {item.type}
                 </span>
                 <span className="px-2.5 py-1 rounded-full border border-zinc-200 dark:border-zinc-700 text-[11px] font-medium text-zinc-600 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-800/50">
                   {item.category}
               ...
               <div className="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800">
                 ...
                 <div className="flex items-center gap-2">
                   {/* PERBAIKAN: Menambahkan cursor-pointer ke tombol-tombol ikon */}
                   ...
                   <a href={item.credentialUrl} target="_blank" rel="noreferrer" className="flex items-center justify-center w-8 h-8 rounded-full border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors cursor-pointer" title="View Credential">
          ...
          <div className="relative w-full max-w-4xl bg-white dark:bg-zinc-950 rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="w-full md:w-2/5 p-6 sm:p-8 flex flex-col">
              ...
              <div className="flex flex-col gap-5 py-6 flex-1">
                <div><p className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider mb-1">{d.lblCredentialId}</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{selectedItem.credentialId}</p></div>
              ...
              <a href={selectedItem.credentialUrl} target="_blank" rel="noreferrer" className="mt-auto w-full flex items-center justify-center gap-2 bg-zinc-950 dark:bg-white text-white dark:text-zinc-950 py-3 rounded-xl text-sm font-bold hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors cursor-pointer">
                {d.btnCredentialUrl}
